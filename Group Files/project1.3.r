...
no_outlier <- filter(with_outlier, cholesterol < max(carotene_data$cholesterol))
...
# doing stepwise selection with BIC as criteorion (log)
# sorry for the confusion of dataset corruption :)

# Full model (like model3a, on two different datasets)
full_model_no_outlier <- lm(log(betaplasma) ~ ., data = no_outlier)
full_model_with_outlier <- lm(log(betaplasma) ~ ., data = with_outlier)

# Automatic selection and the two resulting models 
stepwise_bic <- step(full_model_with_outlier, direction = "both", k = log(nrow(with_outlier)))
stepwise_bic_no_outlier <- step(full_model_no_outlier, direction = "both", k = log(nrow(no_outlier)))

#coeff est. and conf intervals
stepwise_bic$coefficients
stepwise_bic_no_outlier$coefficients

confint(stepwise_bic)
confint(stepwise_bic_no_outlier)


# 5a #

# we only need to look at reduced dataset
# looking at confidence interval for vitusseldom we see that 0 is in the interval, so
# no need for it and our model will only include vituseno (so are you using vitamins or not)
# Test: H0: beta(vituseseldom) = 0, agains H1: beta(vituseseldom) not equal to 0
...
# Remember to do this for all 5 final models:
...
# To decide which models is best, good to look at adj R^2